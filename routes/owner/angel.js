...
angelRouter.post('/call', async (req, res) => { 
  ...
  try{
    ...
    let start = [start_date[0], start_date[1], start_date[2], start_time[0], start_time[1], start_time[2]];
    let end = [end_date[0], end_date[1], end_date[2], end_time[0], end_time[1], end_time[2]];
    ...
    for (let worker of workers){
        dist = getDistance(worker['latitude'], worker['longitude'], store_lat, store_lon);
        if (dist < 4000){
