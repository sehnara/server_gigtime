...
employmentRouter.post('/', async (req, res, next) => { 
  ...
  try {
    ...
    await con.query(sql, [store_id, request_date, job_id, req.body['description'], req.body['price'], 0])
...
employmentRouter.use('/', async (req, res, next) => {
    ...
    try {
        ...
        const [result] = await con.query(sql, masageDate.masageDateToYearMonthDayHourMinSec(req.body['request_date']));
...
employmentRouter.use('/', async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < day; i++) {
            ...
            insert_array.push([order_id, masageDate.masageDateToYearMonthDay(date), masageDate.masageDateToYearMonthDay(date)+' '+all_hours[j]])
